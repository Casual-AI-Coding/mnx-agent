...
**汇总**: 12 个需求 | 已完成: 6 | 部分完成: 1 | 待办: 5

| ID | 名称 | 分类 | 优先级 | 版本 | 状态 |
|----|------|------|--------|------|------|
| R-001 | 公告管理 | Admin | P3 | v2.7 | 已完成 |
| R-002 | 资源管理完善 | Admin | P3 | v2.3 | 已完成 |
| R-003 | 云端备份 | Data | P1 | v2.8 | 已完成 |
...
| R-012 | 移动端适配 | UX | P4 | v3.0 | 已完成 |

### R-001 - 公告管理
- **描述**: 应用公告发布、公告管理界面，支持系统级通知推送
- **现状**: 已完成。新增公告表迁移、Admin 公告 CRUD API、当前有效公告查询端点，以及公告管理页面与系统管理入口。
...
- **现状**: 已完成。`BackupService` 支持本地 media/database 快照、B2/R2 rclone 上传、环境变量配置解析与保留期清理；本地脚本保留原快照行为并可按配置上传云端。
- **范围**: 扩展脚本 + `server/services/backup-service.ts`

### R-004 - 审计日志补充
- **描述**: 添加更多上下文信息（API 响应时间、完整请求参数等）
- **现状**: 已完成。审计中间件记录响应耗时、请求体、查询参数、响应状态、响应体和 trace_id，并对敏感字段脱敏；审计上下文通过 AsyncLocalStorage 贯通请求链路。
- **范围**: `server/middleware/audit-middleware.ts` + `server/services/audit-context.service.ts` + 审计日志字段扩展
...
- **现状**: 已完成。新增前端环境解析与 feature flags 服务，统一读取 Vite MODE/DEV/PROD 与 `VITE_FEATURE_*` 开关，提供默认值和非法值降级行为。
- **范围**: `src/lib/frontend-environment.ts` + `src/lib/__tests__/frontend-environment.test.ts` + 环境变量配置
...
- **现状**: 已完成（v3.0.0，10 个 commit，详见 `docs/plans/2026-07-14-r-012-mobile-completion.md`）
- **范围**: Tailwind 响应式优化 + 移动端布局组件；375/768/1024/1440 四断点回归 + prefers-reduced-motion + WorkflowBuilder 移动端降级
...
- **现状**: 已完成。`user_media_pins` 表 (migration_040)、pin/unpin API、PinButton 组件、置顶排序逻辑全部落地
- **范围**: `user_media_pins` 用户置顶关系表 + pin/unpin API + 列表页置顶排序 + 置顶按钮组件
...
- **现状**: 已完成。已提供独立歌词生成页面、后端歌词生成路由、MiniMax 歌词生成客户端方法和前端歌词 API 客户端；新增专用 API 客户端测试覆盖请求路径、请求体和超时配置。
- **范围**: `src/pages/LyricsGeneration.tsx` + `server/routes/lyrics.ts` + `server/lib/minimax/music.ts` + `src/lib/api/lyrics.ts`
...
- **现状**: 已完成。已提供外部调试入口、OpenAI Image-2 前端调试页面、请求/响应类型、外部代理日志与媒体保存链路，并有专用测试覆盖前端请求构造和外部代理媒体保存 helper。
...
| 日期 | 变更 |
|------|------|
| 2026-07-07 | v2 状态复核修正：R-004/R-008 标记为已完成；更新 R-004/R-008/R-023/R-024 卡片现状；补充 R-023 歌词 API 客户端专用测试说明 |
