...
class TestableWorkflowEngine extends WorkflowEngine {
  ...
  testExecuteTransformNode(config: Record<string, unknown>, nodeOutputs: Map<string, unknown>): Promise<unknown> {
    return (this as unknown as { executeTransformNode(c: Record<string, unknown>, n: Map<string, unknown>): Promise<unknown> })
      .executeTransformNode(config, nodeOutputs)
  }
}
...
describe('XSS Prevention in Transform Node', () => {
  ...
  describe('map transform', () => {
    it('should sanitize HTML in mapFunction to prevent XSS', async () => {
      ...
      const result = await engine.testExecuteTransformNode(config, nodeOutputs)
