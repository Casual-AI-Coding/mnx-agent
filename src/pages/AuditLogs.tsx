...
export default function AuditLogs() {
  ...
  useEffect(() => {
    ...
    if (!hasInitializedRef.current) {
      ...
      loadData()
      loadAllPaths()
      loadAllUsers().finally(() => { isFetchingRef.current = false })
    }
  }, [isHydrated])
  ...
  useEffect(() => {
    ...
    isFetchingRef.current = true
    loadData().finally(() => { isFetchingRef.current = false })
  ...
  const loadData = async () => {
    ...
    } catch { toastError('加载失败', '无法获取审计日志') }
    finally { setIsLoading(false) }
  }

  const formatTime = (dateStr: string) =>
    new Date(dateStr).toLocaleString('zh-CN', { month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit', second:'2-digit' })
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <AuditStatCard title={t('audit.totalLogs', '总记录数')} value={stats.total} icon={Shield} color={status.info.icon} />
          <AuditStatCard title={t('audit.avgDuration', '平均耗时')} value={formatDuration(stats.avgDuration)} icon={Clock} color={status.warning.icon} />
          <AuditStatCard title={t('audit.successRate', '成功率')} value={`${((stats.byResponseStatus.filter(s => s.response_status >= 200 && s.response_status < 300).reduce((sum, s) => sum + s.count, 0) / Math.max(stats.total, 1)) * 100).toFixed(1)}%`} icon={CheckCircle2} color={status.success.icon} />
          <AuditStatCard title={t('audit.errorCount', '错误数')} value={stats.byResponseStatus.filter(s => s.response_status >= 400).reduce((sum, s) => sum + s.count, 0)} icon={AlertCircle} color="text-destructive" />
        </div>
      )}
      ...
      <Card className="border-border overflow-hidden">
        <AuditFilterBar
          ...
          onSortByCreatedAt={() => sortBy === 'created_at' ? setSortOrder(o => o === 'asc' ? 'desc' : 'asc') : (setSortBy('created_at'), setSortOrder('desc'))}
          onSortByDuration={() => sortBy === 'duration_ms' ? setSortOrder(o => o === 'asc' ? 'desc' : 'asc') : (setSortBy('duration_ms'), setSortOrder('desc'))}
